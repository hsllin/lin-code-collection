...
import static com.lin.bean.EnumCacheType.INCREASE_CHANGE;
...
//        8204
        ...
        List<IntradayChange.DataBean.AllstockBean> allstockBeanList = bean.getData().getAllstock();
        HashMap<String, Integer> countMap = new HashMap<>();
        //考虑到第一页可能数据还会变的，第二页的数据往后的数据可能是确定了不会变的，所以保留第一页一直查吧
        for (int i = 1; i < 200; i++) {
            param.put("pageindex", i);
            String tempResult = "";
...
//        }
        ...
        try {
            ...
            for (String type : types) {
                ...
                for (int i = 1; i <= 200; i++) {
                    ...
                    try {
                        iStringRedisRepository.remove(INCREASE_CHANGE, cacheKey);
                    } catch (Exception e) {
                        System.err.println("清理缓存键失败: " + cacheKey + ", 错误: " + e.getMessage());
                        allCleared = false;
                    }
                }
            }
     ...
     * @return 是否清理成功
     */
    public boolean cleanCachedByType(String type) {
        try {
            String tradeDay = CommonUtils.getTradeDay(0);
            String baseKey = "IntradayChange_" + type + "_" + tradeDay;
            boolean allCleared = true;
            ...
            System.err.println("清理类型缓存失败: " + e.getMessage());
            return false;
        }
    }
     ...
     * @return 是否存在
     */
    public boolean isCacheExists(String type, int pageIndex) {
        try {
            String tradeDay = CommonUtils.getTradeDay(0);
            String cacheKey = "IntradayChange_" + type + "_" + tradeDay + "_" + pageIndex;
            return iStringRedisRepository.getString(INCREASE_CHANGE, cacheKey).isPresent();
        } catch (Exception e) {
            System.err.println("检查缓存失败: " + e.getMessage());
            return false;
        }
    }
