...
public class PopularStockService {
    ...
    public void downLoadHotBoardAndConceptData() {
        try {
            ...
            List<IndustryPopularStock.DataDTO.PlateListDTO> list = getPopularConceptList("concept");
            list.addAll(getPopularConceptList("industry"));
            for (IndustryPopularStock.DataDTO.PlateListDTO dataBean : list) {

                bw.write(dataBean.getCode() + " " + dataBean.getName() + " " +dataBean.getRiseAndFall()+"%");
                bw.newLine();
            }
